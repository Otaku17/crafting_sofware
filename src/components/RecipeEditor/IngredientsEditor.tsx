...
import { SearchSelect, Input } from '../layout/Form';
...
export const IngredientsEditor: React.FC<Props> = ({ recipeKey }) => {
  const { lang, config, items, itemIcons, deleteIngredient, updateIngredient } =
    useStore();
      ...
      {entries.map(([item, qty]) => (
        <div key={item} className={styles.row}>
          <SearchSelect
            ...
            value={item}
            onChange={(e) =>
              updateIngredient(recipeKey, item, e.target.value, null)
            }
          ...
          <Input
            ...
            min={1}
            onChange={(e) =>
              updateIngredient(recipeKey, item, null, parseInt(e.target.value))
            }
          />
          <button
